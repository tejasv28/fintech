...
import { ArrowLeft, User, DollarSign, Briefcase, FileText, CheckCircle2, XCircle } from 'lucide-react';
      ...
      <div className="flex justify-between items-center bg-surface-card p-6 rounded-card shadow-card border border-border">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink-900 flex items-center">
            Application <span className="font-mono text-ink-300 font-normal ml-3 text-lg">#{app.id.substring(0,8)}</span>
          </h1>
          <p className="text-sm text-ink-500 mt-1">Submitted on <span className="font-mono">{formatDate(app.createdAt)}</span></p>
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-surface-card rounded-card shadow-card border border-border p-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div>
                ...
                <p className="font-mono text-lg font-semibold text-ink-900">{formatCurrency(app.loanAmount)}</p>
              </div>
              <div>
                <p className="text-[13px] font-medium text-ink-500 mb-1">Loan Term</p>
                <p className="font-mono text-base font-medium text-ink-900">{app.loanTermMonths} <span className="font-sans text-sm font-normal text-ink-500">Months</span></p>
              </div>
          ...
          <div className="bg-surface-card rounded-card shadow-card border border-border p-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div>
                ...
                <p className="font-mono text-base font-medium text-ink-900">{formatCurrency(app.annualIncome)}</p>
              </div>
              <div>
                <p className="text-[13px] font-medium text-ink-500 mb-1">Employment Status</p>
                <p className="font-medium text-ink-900">{app.employmentStatus?.replace('_', ' ')}</p>
              </div>
              <div>
                <p className="text-[13px] font-medium text-ink-500 mb-1">Debt-to-Income Ratio</p>
                <p className="font-mono text-base font-medium text-ink-900">{app.debtToIncomeRatio}%</p>
              </div>
              <div>
                <p className="text-[13px] font-medium text-ink-500 mb-1">Credit Score</p>
                <p className={`font-mono text-base font-bold ${app.creditScore < 650 ? 'text-danger' : 'text-success'}`}>
                  {app.creditScore}
                </p>
              </div>
            </div>
            ...
            <div className="bg-surface-card rounded-card shadow-card border border-border p-6">
              ...
              <div className="space-y-3">
                {ruleResults.map((rule, idx) => (
                  <div key={idx} className={`p-4 rounded-lg border-l-4 bg-surface flex items-start justify-between ${rule.passed ? 'border-l-success' : 'border-l-danger'}`}>
                    <div>
                      <h4 className="text-sm font-semibold text-ink-900">{rule.ruleName}</h4>
                      <p className="text-[13px] text-ink-700 mt-1">{rule.message}</p>
                    </div>
                    {rule.passed ? <CheckCircle2 className="h-5 w-5 text-success shrink-0" /> : <XCircle className="h-5 w-5 text-danger shrink-0" />}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-surface-card rounded-card shadow-card border border-border p-8 flex flex-col items-center">
            <h2 className="font-display text-base font-semibold text-ink-900 w-full text-center mb-8">Risk Assessment</h2>
            <RiskGauge score={app.riskScore || 0} />
            
            {app.rejectionReason && (
              <div className="mt-8 p-4 bg-danger-soft border border-danger/20 rounded-xl w-full">
                <p className="text-xs uppercase tracking-wider font-semibold text-danger mb-1">System Note</p>
                <p className="text-sm font-medium text-danger/90 leading-relaxed">{app.rejectionReason}</p>
              </div>
            )}
          </div>
          ...
          {isActionable && (
            <div className="bg-surface-card rounded-card shadow-card border border-border p-6">
              <h2 className="font-display text-base font-semibold text-ink-900 mb-5">Officer Actions</h2>
                ...
                <div className="space-y-3">
                  ...
                  {!showRejectInput ? (
                    ...
                    <div className="space-y-3 mt-5 pt-5 border-t border-border">
                      <label className="block text-[13px] font-medium text-ink-700">Rejection Reason</label>
                      <textarea
                        value={rejectReason}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <div className="bg-surface-card rounded-card shadow-card border border-border p-6">
          ...
          <div className="space-y-5">
            {timeline.map((event, index) => (
              <div key={event.id} className="relative pl-7 pb-5 border-l-2 border-border last:pb-0 last:border-0">
                <div className="absolute w-3.5 h-3.5 bg-accent rounded-full -left-[8px] top-1 border-2 border-white"></div>
                <p className="text-[13px] font-semibold uppercase tracking-wider text-ink-900">{event.eventType.replace(/_/g, ' ')}</p>
                <p className="font-mono text-xs text-ink-300 mt-0.5 mb-1.5">{formatDate(event.createdAt)}</p>
        ...
        <div className="bg-surface-card rounded-card shadow-card border border-border p-6 flex flex-col h-full">
          ...
          <div className="space-y-4 mb-5 flex-1 overflow-y-auto max-h-[300px] pr-2">
            {notes.length === 0 ? <p className="text-ink-500 text-sm">No internal notes.</p> : notes.map(note => (
              <div key={note.id} className="bg-surface p-4 rounded-xl border border-border">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-mono text-[11px] text-ink-300">{formatDate(note.createdAt)}</span>
                </div>
                <p className="text-sm text-ink-700 leading-relaxed">{note.content}</p>
              </div>
            ))}
          </div>
          <form onSubmit={handleAddNote} className="flex flex-col space-y-3 mt-auto pt-4 border-t border-border">
            <textarea
